...
export default function CreateMaterialsKitsPage() {
  ...
  function changeQuantity(id, quantidade) {
    ...
    for (let i = 0; i < materiaisList.length; i++) {
      if (
        materiaisList[i].id === id &&
        quantidade <= materiaisList[i].quantidade
      ) {
        console.log("quantidade => ", materialsKitList);
        materialsKitList[counter].quantidade = quantidade;
  ...
  return (
    <div>
      ...
      <form>
        ...
        <div>
          <label>Search </label>
          <input type="search" onChange={searchData} id="" />
        </div>
          ...
          <ul>
            {
              ...
              materiaisList?.map((materialsAtributes) => (
                <ListMaterialItem
                  materialsAtributes={materialsAtributes}
                  id={1}
                  evento={evento}
                />
              ))
          ...
          <ul>
            {
              ...
              materialsKitList?.map((materialsKitListAtributes) => (
                <ListMaterialItem
                  materialsAtributes={materialsKitListAtributes}
                  id={2}
                  evento={removeMaterial}
                  evento2={changeQuantity}
                />
              ))
            }
          </ul>
